...
public:
  ...
  bool begin_or_resume_upload(const FileUploadResumeState &requested,
  ...
  bool append_upload_chunk(
      ...
      std::uint64_t &
          accepted_offset, // 写入后,服务器磁盘中新的文件大小,告诉客户端当前文件大小
      std::string &error);

  bool append_upload_bytes(const std::filesystem::path &temp_path,
                           std::uint64_t expected_offset, const char *bytes,
                           std::size_t byte_count,
                           std::uint64_t &accepted_offset, std::string &error);
  ...
  std::string make_offer_line(std::uint64_t transfer_id,
                              const FileTransferMetadata &metadata) const;
...
private:
  ...
  void deliver_file(std::uint64_t transfer_id,     // 传输的ID
  ...
  std::filesystem::path upload_part_path(const std::string &token) const;

  std::filesystem::path upload_meta_path(const std::string &token) const;

  bool write_resume_state(const std::filesystem::path &path,
                          const FileUploadResumeState &state,
                          std::string &error) const;

  bool read_resume_state(const std::filesystem::path &path,
                         FileUploadResumeState &state,
                         std::string &error) const;

  static bool same_resume_identity(const FileUploadResumeState &left,
