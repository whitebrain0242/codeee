...
bool configureTcpKeepAlive(
    ...
    if (!setIntOption(socketFd, SOL_SOCKET, SO_KEEPALIVE, 1, "SO_KEEPALIVE", error))
        return false;
    
    if (!setIntOption(socketFd, IPPROTO_TCP, TCP_KEEPINTVL, intervalSeconds, "TCP_KEEPINTVL", error))
        return false;
    
    if (!setIntOption(socketFd, IPPROTO_TCP, TCP_KEEPIDLE, idleSeconds, "TCP_KEEPIDLE", error))
        return false;
    
    if (!setIntOption(socketFd, IPPROTO_TCP, TCP_KEEPCNT, probeCount, "TCP_KEEPCNT", error))
        return false;
