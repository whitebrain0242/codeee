...
TcpServer::TcpServer(EventLoop *loop, const sockaddr_in &listenAddress,
                     std::string name)
    : loop_(loop), name_(std::move(name)),
      ...
      started_(false), nextConnectionId_(1) {
...
void TcpServer::start() {
  bool expected = false;
  // 使用 compare_exchange_strong 原子地将 started_ 从 false 改为 true，若已经为
  // true 则直接返回（防止重复启动）。
...
void TcpServer::newConnection(int socketFd, const sockaddr_in &peerAddress) {
  ...
  if (::getsockname(socketFd, reinterpret_cast<sockaddr *>(&localAddress),
                    &addressLength) < 0) {
    ::close(socketFd);
