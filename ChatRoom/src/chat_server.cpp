...
void ChatServer::handle_private_message(
    ...
    payload.set_sender_username(
        sender
    );
    payload.set_recipient_username(
        target
    );
    payload.set_content(
        message
    );
    payload.set_created_at_unix_ms(
        now_unix_ms()
    );

    {
        ...
        if (decision ==
            DirectMessageDecision::DatabaseError) {
            database_error(
                connection,
                "checking private-message policy",
                error
            );
            return;
        }
        ...
        if (decision ==
            DirectMessageDecision::TargetMissing) {
            connection->send(
        ...
        if (decision ==
            DirectMessageDecision::NotFriends) {
            connection->send(
        ...
        if (decision ==
            DirectMessageDecision::BlockedByRecipient) {
            connection->send(
                "[error] recipient privacy settings "
    ...
    if (!database_.is_friend_blocked(
        ...
        std::cerr
            << "failed to recheck block before live delivery of #"
            << message_id
            << ": "
            << error
            << '\n';
...
void ChatServer::handle_file_end(
    ...
    if (upload.scope ==
        ...
        if (decision !=
            DirectMessageDecision::Allowed) {
            file_transfer_service_.cancel_upload(
                upload.token
            );

            std::string reason =
                "private-file permission changed "
                "during upload";

            if (decision ==
                DirectMessageDecision::BlockedByRecipient) {
                reason =
                    "recipient privacy settings now block "
                    "direct files from you";
            } else if (decision ==
                       DirectMessageDecision::NotFriends) {
                reason =
                    "recipient is no longer your friend";
            } else if (decision ==
                       DirectMessageDecision::TargetMissing) {
                reason =
                    "recipient account no longer exists";
                       ...
                       DirectMessageDecision::DatabaseError) {
                reason =
                    "database error while rechecking "
...
void ChatServer::deliver_file_to_user(
    ...
    if (transfer.metadata.scope() ==
        ...
        if (!database_.is_friend_blocked(
            ...
            std::cerr
                << "failed to check file block policy for #F"
                << transfer.id
                << ": "
                << block_error
                << '\n';
...
bool ChatServer::is_user_online(
    ...
    return online_users_.is_online(
        username
    );
}
...
void ChatServer::broadcast_to_logged_in(
    ...
    const std::vector<TcpConnectionPtr>
        recipients =
            online_users_.connections(
                except
            );

    for (const TcpConnectionPtr& recipient :
         recipients) {
        recipient->send(
            message
        );
    ...
    if (!database_.pending_private_messages(
        ...
        for (const StoredMessage& message :
            ...
            if (!database_.is_friend_blocked(
                ...
                std::cerr
                    << "failed to recheck offline private block for #"
                    << message.id
                    << ": "
                    << block_error
                    << '\n';
