...
MYSQL *open_connection(const MySqlConfig &config, std::string &error) {
...
MySqlConnectionPool::Lease::Lease(MySqlConnectionPool *owner, MYSQL *connection)
    : owner_(owner), connection_(connection) {}

MySqlConnectionPool::Lease::~Lease() { reset(); }

MySqlConnectionPool::Lease::Lease(Lease &&other) noexcept
    : owner_(other.owner_), connection_(other.connection_) {
  other.owner_ = nullptr;
  other.connection_ = nullptr;
}

MySqlConnectionPool::Lease &
MySqlConnectionPool::Lease::operator=(Lease &&other) noexcept {
  if (this == &other) {
    return *this;
  }
...
MYSQL *MySqlConnectionPool::Lease::get() const noexcept { return connection_; }
...
MySqlConnectionPool::~MySqlConnectionPool() { shutdown(); }

bool MySqlConnectionPool::initialize(const MySqlConfig &config,
                                     std::string &error) {
  shutdown();
  ...
  for (unsigned int i = 0U; i < config.pool_size; ++i) {
    MYSQL *connection = open_connection(config, error);
...
MySqlConnectionPool::Lease MySqlConnectionPool::acquire(std::string &error) {
...
void MySqlConnectionPool::shutdown() {
...

  {
    ...
    cv_.wait(lock, [this] { return available_.size() == all_.size(); });
...
void MySqlConnectionPool::release(MYSQL *connection) noexcept {
  if (connection == nullptr) {
    return;
  }
