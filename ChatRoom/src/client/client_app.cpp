...
int ClientApp::run(const ClientAppConfig &config) {
  ...
  while (running) {
    ...
    if (!waiting_for_server_close && (descriptors[0].revents & POLLIN)) {
      std::string line;
      ...
      remember_login_attempt(line);

      const bool quitting = trim(line) == "QUIT";
    ...
    if (!heartbeat_.tick(transport_, heartbeat_error)) {
      std::cerr << "[heartbeat] " << heartbeat_error
                << "; closing connection.\n";
      break;
    }
  }
  ...
  if (!server_buffer_.empty()) {
    process_server_line(server_buffer_);
  }
