...
bool load_key_values(
    ...
    while (std::getline(input, line)) {
        ...
        if (equals == std::string::npos) {
            error = "invalid " + label + " config line " + std::to_string(line_number);
            return false;
        }
...
//加载数据库配置：存储聊天记录
bool load_mysql_config(const std::string& path, MySqlConfig& config, std::string& error) {
    std::unordered_map<std::string, std::string> values;
    if (!load_key_values(path, "MySQL", values, error)) return false;

    // 字符串字段：直接覆盖（如果配置里有）
    if (values.count("host")) config.host = values["host"];
    if (values.count("user")) config.user = values["user"];
    if (values.count("password")) config.password = values["password"];
    if (values.count("database")) config.database = values["database"];

    // 端口校验（1 ~ 65535）
    if (values.count("port")) {
        unsigned int parsed = 0;
        if (!parse_unsigned(values["port"], parsed) || parsed == 0U || parsed > 65535U) {
    ...
    if (config.database.empty()) { error = "MySQL config requires database"; return false; }
    return true;
}

//管理在线状态
bool load_redis_config(const std::string& path, RedisConfig& config, std::string& error) {
    std::unordered_map<std::string, std::string> values;
    ...
    if (values.count("connect_timeout_ms")) {
        unsigned int parsed = 0;
        if (!parse_unsigned(values["connect_timeout_ms"], parsed) ||
            parsed == 0U || parsed > 30000U) {
    ...
    if (config.host.empty()) { error = "Redis config requires host"; return false; }
    if (config.key_prefix.empty()) { error = "Redis config requires key_prefix"; return false; }
    if (config.server_name.empty()) { error = "Redis config requires server_name"; return false; }
    return true;
}
//服务端证书
bool load_tls_server_config(const std::string& path, TlsServerConfig& config, std::string& error) {
    std::unordered_map<std::string, std::string> values;
    ...
    if (values.count("certificate_file")) config.certificate_file = values["certificate_file"];
    if (values.count("private_key_file")) config.private_key_file = values["private_key_file"];
...
//客户端验证服务端证书
bool load_tls_client_config(const std::string& path, TlsClientConfig& config, std::string& error) {
    std::unordered_map<std::string, std::string> values;
    ...
    if (values.count("ca_file")) config.ca_file = values["ca_file"];
    if (values.count("server_name")) config.server_name = values["server_name"];
