...
Acceptor::Acceptor(EventLoop *loop, const sockaddr_in &listenAddress)
    : loop_(loop), acceptSocket_(::socket(
                       AF_INET, SOCK_STREAM | SOCK_NONBLOCK | SOCK_CLOEXEC, 0)),
      acceptChannel_(nullptr), listening_(false) {
  if (acceptSocket_ < 0) {
    throwSocketError("socket");
  }

  int enabled = 1;
  if (::setsockopt(acceptSocket_, SOL_SOCKET, SO_REUSEADDR, &enabled,
                   sizeof(enabled)) < 0) {
    ::close(acceptSocket_);
    throwSocketError("setsockopt(SO_REUSEADDR)");
  }

  if (::bind(acceptSocket_, reinterpret_cast<const sockaddr *>(&listenAddress),
...
void Acceptor::handleRead() {
  ...
  while (true) {
    ...
    const int socketFd =
        ::accept4(acceptSocket_, reinterpret_cast<sockaddr *>(&peerAddress),
                  &addressLength, SOCK_NONBLOCK | SOCK_CLOEXEC);
