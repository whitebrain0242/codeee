...
std::string hash_password_pbkdf2(const std::string &password) {
  ...
  // 执行 PBKDF2
  if (!derive(password, salt.data(), salt.size(), kIterations, hash.data(),
              hash.size()))
...
bool verify_password_pbkdf2(const std::string &password,
  ...
  if (iterations < 10000 || iterations > 2000000)
    return false; // 防御性范围检查

  // 3. 从十六进制还原盐和期望哈希
  std::vector<unsigned char> salt;
  std::vector<unsigned char> expected;
  if (!from_hex(encoded.substr(second + 1, third - second - 1), salt) ||
      !from_hex(encoded.substr(third + 1), expected) || salt.empty() ||
      expected.empty()) {
    return false;
  }
  ...
  if (!derive(password, salt.data(), salt.size(), iterations, actual.data(),
              actual.size())) {
    return false;
  }
